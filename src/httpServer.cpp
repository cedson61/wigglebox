...
void RequestHandler::handleRequest(const Request& req, Reply& rep)
{
    ...
    if (requestPath.find("/capture") == 0)
        _commandQueue.push_back({CommandId::capture, requestArgs});
    else if (requestPath.find("/getCaptureName") == 0)
        _commandQueue.push_back({CommandId::getCaptureName, requestArgs});
